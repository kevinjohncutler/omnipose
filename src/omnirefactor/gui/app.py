...
class Segmenter:
    ...
    def _ensure_model(self) -> None:
        if self._model is None:
            with self._model_lock:
                if self._model is None:
                    from omnirefactor import models  # local import to avoid startup cost

                    self._model = models.OmniModel(
                        gpu=False,
    ...
    def preload_modules_async(self, delay: float = 0.0) -> None:
        ...
        def _target() -> None:
            if delay > 0:
                time.sleep(delay)
            try:
                import omnirefactor.models  # noqa: F401  (import for side effects)
                import omnirefactor.utils  # noqa: F401
    ...
    def _ensure_core(self):
        if self._core_module is None:
            from omnirefactor import core as core_module
    ...
    def _ensure_utils(self):
        if self._utils_module is None:
            from omnirefactor import utils as utils_module
...
def _start_uvicorn_subprocess(
    ...
    *,
    ...
    args = [
        ...
        "-m",
        "uvicorn",
        "omnirefactor.gui.app:create_app",
        "--factory",
        "--host",
        host,
        "--port",
