...
def get_arg_parser():
    ...
    input_img_args.add_argument("--chan2", default=0, type=int,
                                help=("nuclear channel (if cyto, optional); 0: NONE, 1: RED, "
                                      "2: GREEN, 3: BLUE. Default: %(default)s"))
    ...
    model_args.add_argument("--kernel_size", default=2, type=int,
                            help="kernel size for maskpool. Starts at 2, higher means more aggressive downsampling.")
    model_args.add_argument("--scale_factor", default=2, type=int,
                            help="scale factor for upscaling / downscaling")
    model_args.add_argument("--dilation", default=1, type=int,
                            help="dilation for convolutions in model")
    model_args.add_argument("--nsample", default=4, type=int,
    ...
    algorithm_args.add_argument("--cluster", action="store_true",
                                help="DBSCAN clustering. Reduces oversegmentation of thin features (disabled by default)")
    algorithm_args.add_argument("--no_suppress", action="store_true",
                                help=("Euler integration 1/t suppression reduces oversegmentation but can give "
                                      "undersegmentation in 3D; this flag disables it."))
    algorithm_args.add_argument("--fast_mode", action="store_true",
                                help="make code run faster by turning off 4 network averaging and resampling")
    ...
    algorithm_args.add_argument("--no_net_avg", action="store_true",
    ...
    algorithm_args.add_argument("--diameter", required=False, default=0.0, type=float,
    ...
    algorithm_args.add_argument("--mask_threshold", default=0, type=float,
                                help="mask threshold, default is 0, decrease to find more and larger masks")
    algorithm_args.add_argument("--niter", default=None, type=float,
                                help="Number of Euler iterations, enter value to override Omnipose diameter estimation (under/over-segment)")
    ...
    algorithm_args.add_argument("--min_size", default=15, type=float,
                                help="minimum size for masks, helps if small debris is labeled")
    algorithm_args.add_argument("--max_size", default=None, type=float,
                                help="maximum size for masks, helps if background patches are labeled")
    ...
    output_args.add_argument("--savedir", default=None, type=str,
    ...
    training_args.add_argument("--concatenation",
                               default=0, type=int,
                               help=("concatenate downsampled layers with upsampled layers "
    ...
    training_args.add_argument("--no_save", action="store_true",
    ...
    training_args.add_argument("--checkpoint", action="store_true",
                               help="turn on checkpoints to reduce memory usage")
    ...
    development_args.add_argument("--verbose", action="store_true",
                                  help="flag to output extra information (e.g. diameter metrics) for debugging and fine-tuning parameters")
    development_args.add_argument("--testing", action="store_true",
