...
def _ensure_handler():
    global _LOGGER_READY, _LOGGER_FORMATTER
    ...
    if not _LOGGER_READY:
        ...
        _LOGGER_FORMATTER = formatter
        _LOGGER_READY = True
    return _LOGGER_FORMATTER


def set_logger_color(name, hex_color):
    formatter = _ensure_handler()
    formatter.COLORS[name] = hex_to_ansi(hex_color)


def get_logger(name=__name__, color=None):
    if color:
        set_logger_color(name, color)
    _ensure_handler()
...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if immediate_directory in self.COLORS:
            ansi_color = self.COLORS[immediate_directory]
        elif module_name in self.COLORS:
            ansi_color = self.COLORS[module_name]
        else:
            ansi_color = '\033[90m'  # gray if not specified above
        colored_record.msg = (
            ansi_color +
...
class TqdmToLogger(io.StringIO):
    """Redirect tqdm output to logging instead of stdout."""
