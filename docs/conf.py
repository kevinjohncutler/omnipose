...
os.environ.setdefault("OMNIREFACTOR_DOCS_EXPORT_ALL", "1")
...
def _expose_submodule_members(pkg_name: str) -> None:
    """Import submodules and re-export public members on the package."""
    import importlib
    import pkgutil
    import sys
...
def _collect_public_members(pkg_name: str) -> tuple[list[str], list[str]]:
    """Return public (non-underscore) function/class names exposed on pkg_name."""
    import importlib
    import inspect
...
def setup(app):
    def _build_api_pages(_app):
        _strip_all_for_autodoc()
        ...
        # Build API index with a toctree of discovered packages.
        index_lines = [
            "API Reference",
            "=============",
            "",
...
if MINIMAL_DOCS:
    ...
    raw_release = os.popen('git describe --tags 2>/dev/null').read().strip()
    if raw_release:
        release = re.sub('^v', '', raw_release)
    else:
        release = "0"
...
if MINIMAL_DOCS:
    extensions = [ext for ext in extensions if ext != "sphinx.ext.autosummary"]
...
if tracked_paths:
    ...
    for path in docs_dir.rglob("*"):
        ...
        if api_dir in path.parents:
            # API pages are generated at build time and should not be excluded.
            continue
