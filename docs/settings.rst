...
.. tip::
    At this time, the diameter used for training is not saved with the model parameters and therefore must be specified using ``mymodel.diameter=60.0`` after initializing ``mymodel=models.OmniModel()``. 
...
:header-2:`Automatic diameter`
------------------------------

Omnirefactor does not ship a SizeModel-based automatic diameter estimator. Set ``diameter`` explicitly
when you need rescaling, or leave it at ``0`` to disable rescaling.
...
The network is run on your rescaled image -- where the rescaling factor is determined 
