...
# In-kernel coverage instrumentation is auxiliary (feeds the cross-device
# coverage-combine workflow). Only inject it when `coverage` is importable so
# the notebook tests still run when coverage isn't installed.
