...
def _set_platform(monkeypatch, name):
    monkeypatch.setattr(io.paths.platform, "system", lambda: name)


def test_adjust_file_path_macos_rewrites_home(monkeypatch):
    _set_platform(monkeypatch, "Darwin")
    assert io.paths.adjust_file_path("/home/alice/project") == "/Volumes/project"


def test_adjust_file_path_linux_rewrites_volumes(monkeypatch):
    _set_platform(monkeypatch, "Linux")
    home_dir = "/home/tester"
    monkeypatch.setattr(io.paths.os.path, "expanduser", lambda _: home_dir)
    adjusted = io.paths.adjust_file_path("/Volumes/datasets/run1")
...
def _setup_windows(monkeypatch, home_dir=None):
    ...
    monkeypatch.setattr(io.paths.os.path, "normpath", win_normpath)
    return home_dir


def test_adjust_file_path_windows_from_home(monkeypatch):
    home_dir = _setup_windows(monkeypatch)
    adjusted = io.paths.adjust_file_path("/home/alice/project/data.txt")
...
def test_adjust_file_path_windows_from_volumes(monkeypatch):
    home_dir = _setup_windows(monkeypatch)
    adjusted = io.paths.adjust_file_path("/Volumes/share/results")
...
def test_adjust_file_path_windows_home_with_spaces(monkeypatch):
    ...
    adjusted = io.paths.adjust_file_path("/home/alice/OneDrive - Example Company/data")
